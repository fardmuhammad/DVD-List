...
const App = () => {
  ...
  /* On load, get the DVD data in the `./json/dvd.json' file
   * Oh, and don't think I didn't notice the 'Law &amp; Order' when loading in the JSON.
   * The DOMParser fixes that. */
  useEffect(() => {
    const getDvdData = async () => {
      try {
        setLoadingState("loading");
        const response = await fetch("/json/dvd.json");
        ...
        dispatch(initialize({ newList: items as IDvdListObject[] }));
      } catch (e) {
